var dscSigner = (function($) {
    ...
    function initialize(postInitCallback) {
        $.ajax({
            ...
            alert("NICDSign client is not installed or running.If NICDSign client is already installed, \nremove and reconnect the token before trying again.");
        });
        return true;
    }
    ...
    function listCertificatesForSigning() {
        $.ajax({
            ...
            if (data.status === 'success') {
            	if (config.certificateSno == null || config.certificateSno == '') {
            		if(config.certificateData ==null || config.certificateData ==''){
                    	...
                    	for (var i in data.certificates) {
                    		if (config.certificateData === data.certificates[i].certificateData) {
                    			doSign(data.certificates[i].alias, data.certificates[i].serialNumber, data.certificates[i] );
                    			flag = true;
                    		}
                    	}
                    	if (!flag) {
    ...
    function selectCertificate(data) {
        ...
        panel.empty();
        var title = $('<h2/>').text('Certificate for Signing').addClass('jumbotron text-center');
        ...
        for (var i in data.certificates) {
        	...
        	div.append("<table class='table table-bordered table-striped table-hover table-condensed table-responsive center-block' style='width:60%;'>"+
        					"<tbody>"+
        						"<tr><td>Name</td><td style='text-align:left;'>"+data.certificates[i].alias+"</td></tr>"+
        						"<tr><td>Issuer</td><td style='text-align:left;'>"+data.certificates[i].issuer+"</td></tr>"+
        						"<tr><td style='width:100px;'>Valid From</td><td style='text-align:left;'>"+notBeforeDate+"</td></tr>"+
        						"<tr><td>Valid To</td><td style='text-align:left;'>"+notAfterDate+"</td></tr>"+
        					"</tbody>"+
        				"</table>");
        	div.append($('<br><div>').append($('<button/>', {type: 'button', onclick: "dscSigner.doSign(\"" + data.certificates[i].alias + "\", \"" + data.certificates[i].serialNumber + "\", \"" + data.certificates[i].certificateData +"\")"})
                    .text("Confirm Signing").addClass('btn btn-success')));
        }
        panel.append(div);
        panel.show();
        ...
        $.ajax({
            ...
            if (data.status === 'success') {
            	...
            	if (config.postSignCallback)
                	config.postSignCallback(aliasName , data.sign, data.token);
            } else {
                alert(data.errorMessage);
            }
        }).fail(function (jqXHR, textStatus, errorThrown) {
    ...
    function realSignPdfNoStamping(signData) {
        ...
        $.ajax({
            ...
            if (data.status === 'success') {
				$('#panel').hide();
            	if (config.postSignCallback)
                	config.postSignCallback(aliasName , data.signedPdf, data.token);
            } else {
                alert(data.errorMessage);
            }
        }).fail(function (jqXHR, textStatus, errorThrown) {
