...
class DCController extends CI_Controller
{
    use CommonTrait;
    ...
    public function index()
    {
        ...
        //                             AND vill_townprt_code != '00000'
        //                             AND is_signed = 1
        //                             AND user_desig_code = 'ADC'")->result_array();
        ...
        $data['_view'] = 'verification/dc/dashboard';
    ...
    public function getVill() {
        ...
        if($dist_code == '21') {
            ...
            where vd.dist_code='$dist_code' and vd.subdiv_code='$subdiv_code' and vd.cir_code='$cir_code'and vd.mouza_pargona_code='$mouza_pargona_code' and vd.lot_no='$lot_no' and vd.vill_townprt_code!='00000' AND l.status='L' AND (vd.signed_by_lm != '' AND vd.signed_by_lm IS NOT NULL) AND (vd.signed_by_co != '' AND vd.signed_by_co IS NOT NULL) AND (signed_by_adc IS NOT NULL AND signed_by_adc != '') AND (signed_by_dc IS NULL OR signed_by_dc = '');")->result_array();
        }
        else {
            $villages = $this->db->query("select distinct l.mouza_pargona_code, l.lot_no, l.vill_townprt_code,l.loc_name from verified_dags vd join location l on vd.dist_code=l.dist_code and vd.subdiv_code = l.subdiv_code AND vd.cir_code = l.cir_code AND vd.mouza_pargona_code = l.mouza_pargona_code AND vd.lot_no = l.lot_no AND vd.vill_townprt_code= l.vill_townprt_code
            where vd.dist_code='$dist_code' and vd.subdiv_code='$subdiv_code' and vd.cir_code='$cir_code'and vd.mouza_pargona_code='$mouza_pargona_code' and vd.lot_no='$lot_no' and vd.vill_townprt_code!='00000' AND (vd.signed_by_lm != '' AND vd.signed_by_lm IS NOT NULL) AND (vd.signed_by_co != '' AND vd.signed_by_co IS NOT NULL) AND (signed_by_adc IS NOT NULL AND signed_by_adc != '') AND (signed_by_dc IS NULL OR signed_by_dc = '');")->result_array();
        }
        
        $villages = $this->isADCCompletedOfItsDags($dist_code, $subdiv_code, $cir_code, $villages);
    ...
    public function getVillages(){
        ...
        $villages = $this->db->query("select distinct l.mouza_pargona_code, l.lot_no, l.vill_townprt_code,l.loc_name from verified_dags vd join location l on vd.dist_code=l.dist_code and vd.subdiv_code = l.subdiv_code AND vd.cir_code = l.cir_code AND vd.mouza_pargona_code = l.mouza_pargona_code AND vd.lot_no = l.lot_no AND vd.vill_townprt_code= l.vill_townprt_code
        where vd.dist_code='$dist_code' and vd.subdiv_code!='00' and vd.cir_code='$cir_code'and vd.mouza_pargona_code!='00' and vd.lot_no!='00' and vd.vill_townprt_code!='00000' AND (vd.signed_by_lm != '' OR vd.signed_by_lm IS NOT NULL) AND (vd.signed_by_co != '' OR vd.signed_by_co IS NOT NULL) AND (signed_by_adc IS NOT NULL OR signed_by_adc != '') AND (signed_by_dc IS NULL OR signed_by_dc = '');")->result_array();
    ...
    private function isADCCompletedOfItsDags($dist_code, $subdiv_code, $cir_code, $villages){
        ...
        if(count($villages)){
            foreach($villages as $village){
                ...
                $adc_dag_count = $this->db->where($conditions)
                ...
                if($adc_dag_count > 0 && ($adc_dag_count == $is_signed_count)){
                    if(DEVELOPMENT_MODE_STATUS == '1'){
                        $limit = ceil($adc_dag_count * DC_E_SIGN_RANDOMIZE_PERCENTAGE / 100);
                    }else{
                        $limit = ceil($adc_dag_count * DC_E_SIGN_RANDOMIZE_PERCENTAGE / 100);
                    }
        ...
        if(count($verified_dags)){
            foreach($verified_dags as $recordMain){
                ...
                foreach ($records as $record) {
                    ...
                    $actionLink = base_url('index.php/verification/DCController/viewDetails?' .
                        'dist=' . $dist_code .
    ...
    private function getVerifiedDagsDetails($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $village)
    {
        ...
        //         AND (signed_by_adc is not null OR signed_by_adc != '') 
        //         AND date_of_sign_adc IS NOT NULL
        //         AND (signed_by_dc IS NULL OR signed_by_dc = '')
        // ");
    ...
    public function viewDetails()
    {
        ...
        if ($sessionDist != $dist_code) {
            ...
            redirect(base_url() . "index.php/verification/DCController/index");
        }

        $checkIfDagAlreadyVerified = $this->db->query("SELECT * FROM verified_dags WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code=? AND dag_no=? AND (signed_by_lm IS NOT NULL OR signed_by_lm != '') AND (signed_by_sk IS NOT NULL OR signed_by_sk != '') AND (signed_by_co IS NOT NULL OR signed_by_co != '') AND (signed_by_adc IS NOT NULL OR signed_by_adc != '') AND (signed_by_dc IS NOT NULL OR signed_by_dc != '')", [$dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_code, $dag_no])->row();
        ...
        if ($checkIfRecordExistinChithaBasic) {
            ...
            $data['base'] = $this->config->item('base_url');
            $data['signURL'] = base_url('index.php/verification/DCController/getBaseSFile');
            ...
            // $data['verify_check_url'] = base_url('index.php/DCController/isVerified/' . $dist_code .'/'. $subdiv_code . '/' . $circle_code . '/' . $mouza_code . '/' . $lot_no . '/' . $vill_code . '/' . $dag_no);
    ...
    //     $query = "select * from verified_dags where dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? and lot_no=? and vill_townprt_code=? and dag_no=? and (signed_by_lm IS NOT NULL or signed_by_lm != '') and (signed_by_co IS NOT NULL or signed_by_co != '') and (signed_by_adc IS NOT NULL or signed_by_adc != '') and (signed_by_dc IS NULL or signed_by_dc = '')";
    ...
    public function getBaseSFile()
    {
        ...
        if(file_exists(BARAK_VALLEY_CHITHA_FRESH_PDF_DIR . $sub_folder . '/' . $file_name . '.pdf')) {
            ...
            if(empty($check) || $check->signed_by_lm == '' || $check->signed_by_sk == '' || $check->signed_by_co == '' || $check->signed_by_adc == '') {
                $this->db->trans_rollback();
                $response = [
                    'status'=>'FAILED',
                    'responseType'=>1,
            ...
            $adc_name =  $this->db->query("SELECT username FROM users WHERE dist_code=? AND user_code=? AND user_desig_code='ADC'", [$dist_code, $check->signed_by_adc])->row()->username;
            ...
            if($status && $this->db->affected_rows() == 1){
                ...
                $dc_name = $user->username;
                $translate = translateAssToEng($dc_name);
                if(!$translate['success']){
                    $this->db->trans_rollback();
                    $response = [
                        'status'=>'FAILED',
                ...
                $arr = [
                    ...
                    // 'user_name'=> translateAssToEng($user->username),
                    ...
                    'sign_users'=> [
                        ...
                        'adc'=>[
                            'user_name' => $adc_name,
                            'date_of_sign' => date('Y-m-d', strtotime($check->date_of_sign_adc)),
                            'time_of_sign' => date('H:i:s', strtotime($check->date_of_sign_adc))
                        ],
                        'dc'=>[
                            'user_name' => $dc_name,
    ...
    public function viewSignedPdf() {
        ...
        if(isset($_POST['uuid']) && $_POST['uuid']!='') {
            ...
            $data['_view'] = 'verification/dc/view_signed_doc';
            $this->load->view('layout/layout', $data);
        }
    }
    ...
    public function verificationCertificate() {
        ...
        $data['_view'] = 'verification/dc/certificate';
    ...
    public function eSignInit(){
        ...
        if($check_chitha_verification_certificate){
            ...
            $file_name = $dist_code . '_' . $subdiv_code . '_' . $cir_code . '_' .$mouza_pargona_code . '_DC_' . uniqid() . '.pdf';
            ...
            $html = $this->load->view('verification/dc/certificate-partials', $data, TRUE);
            ...
            if($status){
                ...
                $this->db->trans_rollback();
                log_message('error', '#ERRVERCERT0005: Last Query => ' . $this->db->last_query());
                $response = [
                    'status' => 'FAILED',
                    'responseType' => 1,
                    'msg' => '#ERRVERCERT0005: Something went wrong. Please try again later.'
                ];
                echo json_encode($response);
                exit();
            }
    ...
    protected function getVerificationCertificateData(){
        ...
        $data['dc_name'] = $this->db->query("SELECT username FROM users WHERE dist_code=? AND user_code=?",[$dist_code,$user_code])->row()->username;
    ...
    public function verificationCertificateFinal() {
        $this->dataswitch();

        $dist_code = $this->session->userdata('dcode');

        $data = $this->getFinalVerificationCertificateData();
        ...
        $data['is_final_cert_ready_to_esign'] = $this->isFinalCertificateReadyToEsign();
        $data['addhaar_consent_content'] = self::AADHAAR_CONSENT_CONTENT;
        $data['aadhaar_consent_checkbox_text'] = self::AADHAAR_CONSENT_CHECKBOX_TEXT;

        $data['_view'] = 'verification/dc/certificate_final';
        $this->load->view('layout/layout', $data);
    }

    
    public function eSignInitFinalCert(){
        $dist_code = $this->session->userdata('dcode');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $cir_code = $this->session->userdata('cir_code');
        $mouza_pargona_code = $this->session->userdata('mouza_pargona_code');
        ...
        $validationStatus = $this->isFinalCertificateReadyToEsign();
        if(!$validationStatus['success']){
            $response = [
                'status' => 'FAILED',
                'responseType' => 1,
        ...
        if($check_chitha_verification_certificate){
            ...
            $html = $this->load->view('verification/dc/certificate-final-partials', $data, TRUE);
        ...
        $arr = [
            ...
            'user_code' => $this->session->userdata('user_code'),
            'user_desig_code' => $this->session->userdata('user_desig_code'),
            'redirect' => 'verification/DCController/verificationCertificateFinal',
            'is_final' => '1'
        ];
        $param = urlencode(base64_encode(openssl_encrypt(json_encode($arr), "AES-128-CTR", "singleENCRYPT", 0, "1234567893032221")));
    ...
    protected function getFinalVerificationCertificateData(){
        $dist_code = $this->session->userdata('dcode');
    ...
    private function isFinalCertificateReadyToEsign(){
        $dist_code = $this->session->userdata('dcode');
        $status = true;
        $message = '';

        $alloted_dags = $this->db->where('dist_code', $dist_code)
                                    ->where('user_desig_code', 'DC')
                                    ->get('alloted_dags')
                                    ->row();

        if($alloted_dags){
            $not_signed_alloted_dags = $this->db->where('dist_code', $dist_code)
                                    ->where('user_desig_code', 'DC')
                                    ->where('is_signed', 0)
                                    ->get('alloted_dags')
                                    ->row();
            if($not_signed_alloted_dags){
                $status = false;
                $message = 'For some villages E-sign has not been completed';
            }
        }else{
            $status = false;
            $message = 'Alloted Dag has not been completely E-signed from previous designated officers';
        }
