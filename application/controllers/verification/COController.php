...
class COController extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['ajax_url'] = base_url('index.php/verification/COController/getDags');
        $data['_view'] = 'verification/co/dashboard';
    ...
    public function getVillages() {
        ...
        if($dist_code == '21') {
            $villages = $this->db->query("SELECT DISTINCT l.mouza_pargona_code, l.lot_no, l.vill_townprt_code,l.loc_name from verified_dags vd JOIN location l ON vd.dist_code=l.dist_code and vd.subdiv_code = l.subdiv_code AND vd.cir_code = l.cir_code AND vd.mouza_pargona_code = l.mouza_pargona_code AND vd.lot_no = l.lot_no AND vd.vill_townprt_code= l.vill_townprt_code WHERE vd.dist_code='$dist_code' and vd.subdiv_code='$subdiv_code' and vd.cir_code='$cir_code'and vd.mouza_pargona_code='$mouza_pargona_code' and vd.lot_no='$lot_no' AND vd.vill_townprt_code!='00000' AND l.status='L' AND (vd.signed_by_lm != '' AND vd.signed_by_lm IS NOT NULL) AND (vd.signed_by_sk != '' AND vd.signed_by_sk IS NOT NULL) AND (vd.signed_by_co = '' OR vd.signed_by_co IS NULL);")->result_array();
        }
        else {
            $villages = $this->db->query("SELECT DISTINCT l.mouza_pargona_code, l.lot_no, l.vill_townprt_code,l.loc_name from verified_dags vd JOIN location l ON vd.dist_code=l.dist_code and vd.subdiv_code = l.subdiv_code AND vd.cir_code = l.cir_code AND vd.mouza_pargona_code = l.mouza_pargona_code AND vd.lot_no = l.lot_no AND vd.vill_townprt_code= l.vill_townprt_code WHERE vd.dist_code='$dist_code' and vd.subdiv_code='$subdiv_code' and vd.cir_code='$cir_code'and vd.mouza_pargona_code='$mouza_pargona_code' and vd.lot_no='$lot_no' AND vd.vill_townprt_code!='00000' AND (vd.signed_by_lm != '' AND vd.signed_by_lm IS NOT NULL) AND (vd.signed_by_sk != '' AND vd.signed_by_sk IS NOT NULL) AND (vd.signed_by_co = '' OR vd.signed_by_co IS NULL);")->result_array();
        }
    ...
    private function isSKCompletedOfItsDags($dist_code, $subdiv_code, $cir_code, $villages){
        ...
        if(count($villages)){
            foreach($villages as $village){
                ...
                $sk_dag_count = $this->db->where($conditions)
                ...
                if($sk_dag_count > 0 && ($sk_dag_count == $is_signed_count)){
                    $limit = floor($sk_dag_count * CO_E_SIGN_RANDOMIZE_PERCENTAGE / 100);
                    if($limit > 0){
    ...
    public function getDags()
    {
        ...
        if(DEVELOPMENT_MODE_STATUS == '1'){
            ...
            $this->generateAllCOSign($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $village);
        }
        ...
        foreach ($verifiedDags as $recordMain) {
            ...
            foreach ($records as $record) {
                ...
                $actionLink = base_url('index.php/verification/COController/viewDetails?' .
                    'dist=' . $dist_code .
                    '&subdiv=' . $subdiv_code .
                    '&cir=' . $cir_code .
    ...
    private function getVerifiedDagsDetails($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $village)
    {
        ...
        //         AND (signed_by_co IS NULL OR signed_by_co = '')
        // "); 

        return $query->result_array();
    }

    private function generateAllCOSign($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $village){
        $user_code = $this->session->userdata('user_code');
            ...
            foreach($alloted_dags as $alloted_dag){
                $this->db->where('id', $alloted_dag['verified_dags_id'])
                        ->update('verified_dags', [
                                                    'signed_by_co' => $user_code,
                                                    'date_of_sign_co' => date('Y-m-d H:i:s')
                                                ]);
    ...
    public function viewDetails()
    {
        ...
        if ($checkIfRecordExistinChithaBasic) {
            ...
            $data['base'] = $this->config->item('base_url');
            $data['signURL'] = base_url('index.php/verification/COController/getBaseSFile');
            ...
            // $data['verify_check_url'] = base_url('index.php/COController/isVerified/' . $dist_code .'/'. $subdiv_code . '/' . $circle_code . '/' . $mouza_code . '/' . $lot_no . '/' . $vill_code . '/' . $dag_no);
    ...
    //     $query = "select * from verified_dags where dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? and lot_no=? and vill_townprt_code=? and dag_no=? and (signed_by_lm IS NOT NULL or signed_by_lm != '') and (signed_by_co IS NULL or signed_by_co = '')";
    ...
    public function viewSignedPdf() {
        ...
        if(isset($_POST['uuid']) && $_POST['uuid']!='') {
            ...
            $data['_view'] = 'verification/co/view_signed_doc';
            $this->load->view('layout/layout', $data);
        }
    }
    ...
    public function verificationCertificate()
    {
        ...
        $data['_view'] = 'verification/co/certificate';
        $this->load->view('layout/layout', $data);
    }
    ...
    public function eSignInit(){
        ...
        if($check_chitha_verification_certificate){
            ...
            $file_name = $dist_code . '_' . $subdiv_code . '_' . $cir_code . '_' .$mouza_pargona_code . '_CO_' . uniqid() . '.pdf';
            ...
            $html = $this->load->view('verification/co/certificate-partials', $data, TRUE);
            ...
            if($status){
                ...
                $this->db->trans_rollback();
                log_message('error', '#ERRVERCERT0003: Last Query => ' . $this->db->last_query());
                $response = [
                    'status' => 'FAILED',
                    'responseType' => 1,
                    'msg' => '#ERRVERCERT0003: Something went wrong. Please try again later.'
                ];
                echo json_encode($response);
                exit();
            }
    ...
    protected function getVerificationCertificateData(){
        ...
        $data['co_name'] = $this->db->query("SELECT username FROM users WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND user_code=?",[$dist_code, $subdiv_code, $cir_code,$user_code])->row()->username;
