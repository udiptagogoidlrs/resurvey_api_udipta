...
class NcVillageLmController extends CI_Controller
{
    ...
    public function pendingVillages()
    {
        ...
        $method = 'POST';
        $output = callIlrmsApi($url, $method, ['d' => $dist, 's' => $subdiv, 'c' => $circle, 'm' => $mouza, 'l' => $lot, 'logedin_user_code' => $user_code]);
        ...
        try{
            ...
            if ($nc_village == null) {
                ...
                if (!$check_chitha_basic_nc)
                {
                    ...
                    if(!$insert_bhunaksa_dags){
                        throw new Exception( "Error in getting Bhunaksha dags");
                    }
                ...
                $data = $this->getChithaNBhunaksa($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code);
    ...
    /** getChithaNBhunaksa */
	public function getChithaNBhunaksa($dist_code,$subdiv_code,$cir_code,$mouza_pargona_code,$lot_no,$vill_townprt_code){
		//sq meter
		$b = 1337.803776;
		$k = 267.5607552;
		$l = 13.37803776;
		...
		$query = $this->db->select('count(*) as total_dag, SUM(dag_area_b) as total_bigha,
			...
			->where(array(
				'dist_code' => $dist_code, 'subdiv_code' => $subdiv_code, 'cir_code' => $cir_code, '
		...
		if (!$api_output) {
            ...
            throw new Exception("LANDHUB API FAIL");
			// echo "API FAIL";
			// return;
		}
