...
class SvamitvaCardController extends CI_Controller
{
    ...
    public function location()
    {
        ...
        $data['page_title'] = 'Select Location | NC Village Chitha Entry/Edit';
        // $data['page_header'] = 'Location Details For NC Village Chitha Entry';
        $data['breadcrumbs'] = '
        <li class="breadcrumb-item"><a href="#">Data Entry</a></li>
        <li class="breadcrumb-item"><a href="#">NC Village Chitha Entry/Edit</a></li>
    ...
    public function submitLocation()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_userdata('lm_edit', $this->input->post('lm_edit'));
    ...
    public function dagDetails()
    {
        ...
        $data['lm_edit'] = $this->session->userdata('lm_edit');
    ...
    public function occupierEntry()
    {
        ...
        $this->form_validation->set_rules('occupier_guar_relation[]', 'Guardian Relation', 'trim|required|max_length[100]');
        ...
        $this->form_validation->set_rules('property_type_code[]', 'Property Type', 'trim');
        $this->form_validation->set_rules('occupier_evicted[]', 'Occupier Evicted', 'trim|required');
        $this->form_validation->set_rules('spouse_name[]', 'Spouse Name', 'trim|max_length[100]');

        if ($this->form_validation->run() == false) {
            $text = str_ireplace('<\/p>', '', validation_errors());
            $text = str_ireplace('<p>', '<small style="color:red;">', $text);
            $text = str_ireplace('</p>', '</small><br/>', $text);
            echo json_encode(array('msg' => $text, 'st' => 0));
            return;
        } else {
            ...
            $occupier_names = $this->input->post('occupier_name');
            $occupier_guar_names = $this->input->post('occupier_guar_name');
            $occupier_guar_relations = $this->input->post('occupier_guar_relation');
            $addresss = $this->input->post('address');
            $occupier_class_codes = $this->input->post('occupier_class_code');
            $property_type_codes = $this->input->post('property_type_code');
            $occupier_evicteds = $this->input->post('occupier_evicted');
            $marital_statuss = $this->input->post('marital_status');
            $genders = $this->input->post('gender');
            // $categories = $this->input->post('category');
            // $current_occupations = $this->input->post('current_occupation');
            $spouse_names = $this->input->post('spouse_name');
            $other_descriptions = $this->input->post('other_description');
            $approx_property_values = $this->input->post('approx_property_value');
            ...
            foreach ($occupier_names as $key => $occupier_name) {
                $encro_id = $this->RemarkModel->checkencroseid();
                $occupier['encro_id'] = $encro_id;
                $occupier['rmk_type_hist_no'] = $encro_id;
                $occupier['encro_name'] = $occupier_name;
                $occupier['encro_guardian'] = $occupier_guar_names[$key];
                $occupier['encro_guar_relation'] = $occupier_guar_relations[$key];
                $occupier['encro_add'] = $addresss[$key];
                $occupier['encro_class_code'] = $occupier_class_codes[$key];
                $occupier['property_type_code'] = $property_type_codes[$key];
                $occupier['encro_evicted_yn'] = $occupier_evicteds[$key];
                $occupier['marital_status'] = $marital_statuss[$key];
                $occupier['gender'] = $genders[$key];
                // $occupier['category'] = $categories[$key];
                // $occupier['current_occupation'] = $current_occupations[$key];
                $occupier['spouse_name'] = $spouse_names[$key];
                $occupier['other_description'] = $other_descriptions[$key];
                // $occupier['approx_property_value'] = $approx_property_values[$key] ? $approx_property_values[$key] : 0;
    ...
    public function occupierUpdate()
    {
        ...
        $mm = $this->input->post('occupier_evicted_yn');
        if ($mm == 'Y') {
            $this->form_validation->set_rules('occupier_evic_date', 'Occupier Evic Date', 'trim|required');
        }
