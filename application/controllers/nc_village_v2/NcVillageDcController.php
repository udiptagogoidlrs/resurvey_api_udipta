...
class NcVillageDcController extends CI_Controller
{
    ...
    public function dashboard()
    {
        ...
        $dsign_pending_count = $dc_proposal_pending = $reverted_name_change = 0;

        $url = API_LINK_ILRMS . "index.php/nc_village_v2/NcCommonController/apiGetMapsCountByProceedings";
        
        $method = 'POST';
        $output = callIlrmsApi($url, $method, ['d' => $dist_code, 'logedin_user_code' => $user_code, 'proceeding_types' => [1, 2, 3]]);

        if($output && !empty($output->data)){
            $dsign_pending_count = $output->data->proceeding_1;
            $dc_proposal_pending = $output->data->proceeding_2;
            $reverted_name_change = $output->data->proceeding_3;
            $processed_case_count = $output->data->processed_case_count;
        } 
        
        $data['dsign_pending_count'] = $dsign_pending_count;
        $data['dc_proposal_pending'] = $dc_proposal_pending;
        $data['reverted_name_change'] = $reverted_name_change;
        ...
        $data['_view'] = 'nc_village_v2/dc/nc_village_dc_dashboard';
        $this->load->view('layout/layout', $data);
    }

    public function showVillages($proceeding_type = 1)
    {
        if ($this->session->userdata('usertype') != 7) {
            show_error('<svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><g fill="none" stroke="#FF0000" stroke-linecap="round" stroke-width="2"><path d="M12 9v5m0 3.5v.5"/><path stroke-linejoin="round" d="M2.232 19.016L10.35 3.052c.713-1.403 2.59-1.403 3.302 0l8.117 15.964C22.45 20.36 21.544 22 20.116 22H3.883c-1.427 0-2.334-1.64-1.65-2.984Z"/></g></svg> <p>Unauthorized access</p>', "403");
        }
        ...
        if($proceeding_type == 1){
            $data['_view'] = 'nc_village_v2/dc/villages_g';
        }elseif($proceeding_type == 3){
            $data['_view'] = 'nc_village_v2/dc/villages_sm_f';
        }else{
            show_404();
            return;
        }
    ...
    public function getVillagesG()
    {
        ...
        $api_data = ['d' => $dist_code, 'logedin_user_code' => $user_code, 'proceeding_type' => 1];
        if ($subdiv_code) {
            $api_data = $api_data + ['s' => $subdiv_code];
        }
        if ($cir_code) {
            $api_data = $api_data + ['c' => $cir_code];
        }
        if ($mouza_pargona_code) {
            $api_data = $api_data + ['m' => $mouza_pargona_code];
        }
        if ($lot_no) {
            $api_data = $api_data + ['l' => $lot_no];
        }

        $url = API_LINK_ILRMS . "index.php/nc_village_v2/NcCommonController/apiGetMaps";
        $method = 'POST';
        $output = callIlrmsApi($url, $method, $api_data);
        $maps = $output ? json_decode(json_encode($output->data), true) : [];

        $results = [];
        if(count($maps)){
            foreach($maps as $map){
                $query = "select l.loc_name,ncv.lm_verified_at,ncv.status,ncv.application_no,ncv.lm_note,ncv.co_verified,ncv.co_note,ncv.dc_verified,ncv.co_verified_at,ncv.dc_note, ncv.id as nc_village_id from nc_villages ncv join location l on ncv.dist_code = l.dist_code and ncv.subdiv_code = l.subdiv_code and ncv.cir_code = l.cir_code and ncv.mouza_pargona_code = l.mouza_pargona_code and ncv.lot_no = l.lot_no and ncv.vill_townprt_code = l.vill_townprt_code where ncv.uuid=? AND ncv.app_version='V2'";

                $village = $this->db->query($query, [$map['uuid']])->row();
                ...
                $village->merge_village_names = implode(', ', $merge_village_name_arr);
                $village->merge_village_requests = $merge_village_requests;
                $results[] = $village;
            }
        }
    ...
    public function getVillagesSmf(){
        ...
        $query = "select l.loc_name,ncv.lm_verified_at,ncv.status,ncv.application_no,ncv.lm_note,ncv.co_verified,ncv.co_note,ncv.dc_verified,ncv.co_verified_at,ncv.dc_note, ncv.id as nc_village_id,ncv.jds_verified_at,jds_note from nc_villages ncv join location l on ncv.dist_code = l.dist_code and ncv.subdiv_code = l.subdiv_code and ncv.cir_code = l.cir_code
                 and ncv.mouza_pargona_code = l.mouza_pargona_code and ncv.lot_no = l.lot_no and ncv.vill_townprt_code = l.vill_townprt_code and ncv.app_version='V2'";
    ...
    public function showDags($proceeding_status = 1)
    {
        if ($this->session->userdata('usertype') != 7) {
            show_error('<svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><g fill="none" stroke="#FF0000" stroke-linecap="round" stroke-width="2"><path d="M12 9v5m0 3.5v.5"/><path stroke-linejoin="round" d="M2.232 19.016L10.35 3.052c.713-1.403 2.59-1.403 3.302 0l8.117 15.964C22.45 20.36 21.544 22 20.116 22H3.883c-1.427 0-2.334-1.64-1.65-2.984Z"/></g></svg> <p>Unauthorized access</p>', "403");
        }
        ...
        $nc_village = $this->db->query("select * from nc_villages where application_no='$application_no' and dist_code='$dist_code' and app_version='V2'")->row();
        if (!$nc_village) {
            show_404();
            return;
        }
        ...
        $data['proceeding_status'] = $proceeding_status;
        ...
        if(!$output || empty($output->data)){
            // show_404();
            return redirect(base_url('index.php/nc_village_v2/NcVillageDcController/dashboard'));
        }

        $all_cos = getAllCO($dist_code, $nc_village->subdiv_code, $nc_village->cir_code);
        $data['all_cos'] = $all_cos;

        $all_jds = getAllJDS();
        $data['all_jds'] = $all_jds;
        ...
        $data['map'] = $output->data->map_lists;
        $data['old_maps'] = $output->data->old_map_lists;
        ...
        $data['_view'] = 'nc_village_v2/dc/village_g_dags';
        $this->load->view('layout/layout', $data);
    }
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($this->db->affected_rows() > 0) {
                if ($this->db->trans_status() === false) {
                    ...
                    $method = 'POST';
                    $update_data = [ 
                                        'proccess_type' => 'CERTIFY_DRAFT_CHITHA_AND_MAP', 
    ...
    public function revertVillage()
    {
        ...
        $this->form_validation->set_rules('user', 'CO', 'trim|required');
        if ($this->form_validation->run() == false) {
            echo json_encode(array('errors' => validation_errors(), 'st' => 0));
            return;
        }
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($this->db->trans_status() === false) {
                ...
                $nc_village = $this->db->where('application_no', $application_no)->get('nc_villages')->row();
                // $url = API_LINK_ILRMS . "index.php/nc_village_v2/NcCommonController/insertNcVillageRevertDetails";
                $url = API_LINK_ILRMS . "index.php/nc_village_v2/NcCommonController/insertNcVillageDetails";
                $method = 'POST';
                $update_data = [ 
                ...
                if(!count($response) || $response['status'] != 1){
                    $this->db->trans_rollback();
                    log_message("error", '#NCV20005: Unable to update log');

                    echo json_encode(array(
                        'submitted' => 'N',
                        'msg' => '#NCV20005 => Unable to update log',
                    ));
                    return;
                }
        ...
        if ($pdfbase !== false) {
            ...
            if (file_put_contents($pdf_path, $pdf_content) !== false) {
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    $method = 'POST';
                    $update_data = [ 
                                        'proccess_type' => 'SIGN_DRAFT_CHITHA', 
                                        'dist_code' => $nc_village->dist_code,
                                        'subdiv_code' => $nc_village->subdiv_code,
                                        'cir_code' => $nc_village->cir_code,
                                        'mouza_pargona_code' => $nc_village->mouza_pargona_code,
                                        ...
                                        'pre_user_dig' => 'DC', 
                                        'cur_user_dig' => 'DC', 
                                        // 'remark' => $dc_name . ' signed draft chitha'
                                        'remark' => $nc_village->dc_certification
                                    ];
                    $output = $this->NcVillageModel->callApiV2($url, $method, $update_data);
                    $response = $output ? json_decode($output, true) : [];
    ...
    {
        $this->dbswitch();
        $data['_view'] = 'nc_village_v2/dc/nc_villages_reverted';
        $this->load->view('layout/layout', $data);
    }
    ...
    public function showProposalVillages()
    {
        ...
        $method = 'POST';
        $user_resp = callIlrmsApi($url, $method, ['user_desig' => 'DLR']);
        $users = [];
        if(!$user_resp || !empty($user_resp->users)){
            $users = json_decode(json_encode($user_resp->users), true);
        }
        ...
        $data['proposal'] = $this->load->view('nc_village_v2/dc/approval_notification', $data, true);

        $data['approve_proposal'] = $this->db->select('*')
            ->where(array('dist_code' => $dist, 'user_type' => 'DC', 'status' => 'A'))
            ->get('nc_village_proposal')->result();

        $data['_view'] = 'nc_village_v2/dc/proposal_villages';
        $this->load->view('layout/layout', $data);
    }
    // /** getProposalVillages */
    public function getProposalVillages()
    {
        ...
        $query = "select nc.*,l.loc_name,ch.old_vill_name,ch.new_vill_name from nc_villages nc join change_vill_name ch
                on nc.dist_code = ch.dist_code and nc.uuid = ch.uuid
                 join location l on nc.dist_code = l.dist_code and nc.subdiv_code = l.subdiv_code and nc.cir_code = l.cir_code
             and nc.mouza_pargona_code = l.mouza_pargona_code and nc.lot_no = l.lot_no and nc.vill_townprt_code = l.vill_townprt_code where
                 nc.dist_code='$dist' and nc.status ='K' AND app_version='V2'";
    ...
    public function saveProposalPdf()
    {
        ...
        $content = $this->load->view('nc_village_v2/dc/approval_notification', $data, true);
        ini_set("pcre.backtrack_limit", "500000000");
        ini_set('max_execution_time', '0');
        ini_set('memory_limit', '-1');
        include 'vendor/mpdf/vendor/autoload.php';
    ...
    public function storeSignedProposal()
    {
        ...
        $dc_remark = $this->input->post('remark');
        $user = $this->input->post('user');
            ...
            if (file_put_contents($pdf_path, $pdf_content) !== false) {
                ...
                if ($is_exists) {
                    ...
                    $this->db->where('dist_code', $dist)
                        ...
                        ->update('nc_villages', array('status' => 'L', 'dc_proposal' => 'Y', 'dc_proposal_id' => $lastid));
                }
                if ($this->db->trans_status() === false) {
                    $this->db->trans_rollback();
                    ...
                    if(count($case_nos_arr)){
                        foreach($case_nos_arr as $application_no){
                            $nc_village = $this->db->where('application_no', $application_no)->get('nc_villages')->row();
                            $url = API_LINK_ILRMS . "index.php/nc_village_v2/NcCommonController/insertNcVillageDetails";
                            $method = 'POST';
                            $update_data = [ 
