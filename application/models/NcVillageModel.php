...
class NcVillageModel extends CI_Model
{
    public function getchithaDetailsALL($district_code, $subdivision_code, $circlecode, $mouzacode, $lot_code, $village_code, $dag_no_lower, $dag_no_upper)
    {
        ...
        $q = "Select cb.old_dag_no, cb.dag_no,cb.dag_no_int,cb.patta_no,cb.dag_area_b,cb.dag_area_k,cb.dag_area_lc,cb.dag_revenue,
           cb.dag_local_tax,lcc.land_type from chitha_basic_nc AS cb JOIN landclass_code
           ...
           CAST(coalesce(cb.dag_no_int, '0') AS numeric)<=$dag_no_upper and patta_type_code in ($govt_patta_codes) order by CAST(coalesce(cb.dag_no_int, '0') AS numeric)";
        ...
        $q2 = $this->db->query("
         ...
         cb.dag_local_tax,lcc.land_type,lcc.class_code_cat  from chitha_basic_nc AS cb JOIN landclass_code
          AS lcc ON cb.land_class_code = lcc.class_code where Dist_code='$district_code' and Subdiv_code='$subdivision_code'
              and Cir_code='$circlecode' 
          and Mouza_Pargona_code='$mouzacode' and Lot_No='$lot_code' and Vill_townprt_code='$village_code' 
          and CAST(coalesce(cb.dag_no_int, '0') AS numeric)>=$dag_no_lower and 
        ...
        foreach ($dags as $dag) {
            ...
            $dag_no = $dag->dag_no;
            $patta_code = $dag->patta_type_code;
            ...
            $data1[$dag_no] = array(
                ...
                'dag_localtax' => $dag->dag_local_tax,
                'land_type' => $dag->land_type,
                'class_code_cat' => $dag->class_code_cat,
            );
