    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card" id="loc_save">
                <div class="card-body">
                    <?php echo form_open('import_chitha/ImportChithaController/importVillageChithaNencroacher', ['enctype' => "multipart/form-data"]); ?>
                    <div class="col-12 px-0 pb-3">
                        <div class="bg-info text-white text-center py-1">
                            <h5>Select VIllage to Import Occupiers</h5>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="xl">Select CSV File:</label>
                        <input type="file" accept=".csv" name="xl" class="form-control form-control-sm">
                    </div>
...
<script>
    ...
    $('#l').change(function() {
        ...
        var lot = $(this).val();
        $.ajax({
            url: baseurl + "index.php/reports/DagReportController/villagesSvamitva",
            method: "POST",
            data: {
                dis: dis,
                subdiv: subdiv,
            ...
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                    html += '<option value=' + data[i].vill_townprt_code + '>' + data[i].loc_name + ' (' + data[i].vill_townprt_code + ')' +
                        '</option>';
                }
                $('#v').html(html);
            }
