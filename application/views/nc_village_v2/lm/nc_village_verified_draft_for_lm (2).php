...
<div class="col-lg-12 mt-3">
	<div class="card">
		<div class="card-body">
			<?php echo form_open('nc_village_v2/NcVillageCommonController/viewBhunaksaMap'); ?>
						...
						<tbody>
							<?php foreach ($maps as $key => $map_single): ?>
								<tr>
									<td><?=$key + 1?></td>
									<td>
										<a href="<?=base_url() . 'index.php/nc_village_v2/NcVillageLmController/viewUploadedMap?id=' . $map_single->id?>" class="btn btn-info py-2" style="color: white" target="_blank">
											View Map
										</a>
									</td>
								</tr>
...
<script>
	...
	const allSks = <?= json_encode($all_sks) ?>;
	let allSkHtml = `<select class="form-control" name="forward_to_user" id="forward_to_user"><option value="">Select ${SK_LABEL}</option>`;
	if(Object.keys(allSks).length){
		allSks.forEach((allSk) => {
			allSkHtml += `<option value="${allSk.user_code}">${allSk.name}</option>`;
		});
	}
	allSkHtml += `</select>`;
	const vUuid = "<?= $v_uuid ?>";
	...
	function getDags(vill_townprt_code) {
		...
		$.ajax({
			url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/getDags',
			method: "POST",
			async: true,
			dataType: 'json',
			data: {
			...
			success: function(data) {
				...
				if (data.data.length == 0) {
					...
					swal({
						...
						title: data.message ?? "Bhunaksha data not found.",
						icon: "info",
					});
				} else {
					...
					if (data.lm_verified != 'Y') {
						...
						var final_submit = '<table width="100%"><tr>' +
							'<th class="font-weight-bold"> <b>Select ' + SK_LABEL + ' <span class="text-red">*</span></b> </th></tr>' +
							'<tr><td width="100%"> ' + allSkHtml + ' </td></tr>' +
							'<tr><th class="font-weight-bold"> <b>Remark <span class="text-red">*</span></b> </th></tr>' +
		...
		$.confirm({
			...
			buttons: {
				Confirm: {
					...
					action: function() {
						$.ajax({
							url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/updateDag',
							method: "POST",
							async: true,
							dataType: 'json',
							data: {
								...
								if (data.data.length == 0) {
									...
									if (data.lm_verified != 'Y') {
										var final_submit = '<table width="100%"><tr>' +
											'<th class="font-weight-bold"> <b>Select ' + SK_LABEL + ' <span class="text-red">*</span></b> </th></tr>' +
											'<tr><td width="100%"> ' + allSkHtml + ' </td></tr>' +
											'<tr><th class="font-weight-bold"> Remark <span class="text-red">*</span> </th></tr>' +
	...
	function finalSubmit() {
		...
		const forward_to_user = $('#forward_to_user').val();
		if(forward_to_user == ''){
			alert(`Please choose the ${SK_LABEL}`);
			return false;
		}
		...
		$.confirm({
			...
			buttons: {
				Confirm: {
					...
					action: function() {
						$.ajax({
							url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/lmFinalSubmit',
							method: "POST",
							async: true,
							dataType: 'json',
							data: {
	...
	function villDagsChithaButton() {
		...
		$.ajax({
			url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/generateVillageDagsChitha',
			method: "POST",
			async: true,
			dataType: 'json',
			data: {
		...
		$.confirm({
			...
			buttons: {
				Confirm: {
					...
					action: function() {
						$.ajax({
							url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/verifyDagsChithaAndSave',
							method: "POST",
							async: true,
							dataType: 'json',
							data: {
			...
			buttons: {
				Confirm: {
					...
					action: function() {
						...
						$.ajax({
							url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/regenerateChitha',
							method: "POST",
							async: true,
							dataType: 'json',
							data: {
...
<script>
	...
	function syncDagsWithBhunaksha(){
		...
		$.confirm({
			...
			content: 'You will have to re-generate chitha again!',
			type: 'red',
			typeAnimated: true,
			buttons: {
				Confirm: {
					...
					action: function() {
						...
						$.ajax({
							dataType: "json",
							url: '<?=base_url();?>index.php/nc_village_v2/NcVillageLmController/syncDagsWithBhunakshaAgain',
							data: formData,
							type: "POST",
