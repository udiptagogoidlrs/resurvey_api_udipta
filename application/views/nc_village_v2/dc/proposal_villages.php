	...
	function alpineData() {
		return {
			...
			init() {
				...
				// if(users.length > 0){
				// 	this.user = users[0].unique_user_id;
				// }

				$.ajax({
                    url: '<?=base_url();?>index.php/nc_village_v2/NcVillageDcController/getProposalVillages',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: data,
			...
			signProposal() {
				...
				data['dc_note'] = self.dc_note;
				data['user'] = self.user;
                $.ajax({
                    url: '<?=base_url();?>index.php/nc_village_v2/NcVillageDcController/saveProposalPdf',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: data,
                    success: function(data) {
                        ...
                        if (data != null || data != '') {
                            ...
                            self.loadPreviewForSign('<?= base_url() ?>index.php/nc_village_v2/NcVillageDcController/getProposalBase');
			...
			storeSignedProposal(jsonData, sign_key) {
				...
				$.ajax({
					url: '<?= base_url(); ?>index.php/nc_village_v2/NcVillageDcController/storeSignedProposal',
					method: "POST",
					async: true,
					dataType: 'json',
					data: {
			...
			<div class="card">
				<div class="card-body">
						...
						<tbody>
							...
							<template x-for="(village,index) in villages" :key="index">
								...
								>
									...
									<td>
										<a class="btn btn-sm btn-info text-white" x-bind:href=base_url+"index.php/nc_village/NcVillageDcController/showDags?application_no="+village.application_no target=" _blank">View <i class=" fa fa-eye"></i></a>
									</td>
								</tr>
							</template>
					...
					<div class="mt-3">
						<div class="form-group">
                            ...
                            <select x-model="user" class="form-control">
								...
								?>
											<option value="<?= $user['unique_user_id'] ?>"><?= $user['name'] ?></option>
								<?php
										endforeach;
									endif; 
								?>
					...
					<div class="mt-3" x-show="villages.length > 0">
						<button class="btn  btn-md btn-success" x-bind:class="(selected_cases.length == 0 || dc_note.trim() === '' || user === '') ? 'disabled' : ''" x-on:click="(selected_cases.length > 0 && dc_note.trim() !== '' && user !== '') ? approveProposal : 'javascript:void(0)'">
