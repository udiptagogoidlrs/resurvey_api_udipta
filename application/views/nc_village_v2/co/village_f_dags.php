    ...
    function alpineData() {
        return {
            ...
            getDags() {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village_v2/NcVillageCoController/getDags',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village_v2/NcVillageCoController/certifyVillage',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            revertVillage() {
                var self = this;
                this.is_loading = true;
                let optionsHtml = '<option value="">Please Select LRS</option>';
                self.sks.forEach(function(sk) {
                    optionsHtml += '<option value="' + sk.user_code + '">' + sk.name + '</option>';
                });
                let contentHtml =
                    '<form id="remarkForm">' +
                        '<h3>Please confirm to Revert Back this Village to LRS</h3>' + 
                        '<div class="row">' +
                            '<div class="col-lg-12 mb-2">' +
                                '<label class="form-check-label" for="co_user_code">LRS</label>' +
                                '<select id="sk_user_code" name="sk_user_code" class="form-control">' +
                                    optionsHtml+
                                '</select>' +
                            '</div>'  +
                            '<div class="col-lg-12">' +
                                '<label class="form-check-label" for="sk_remark">Note</label>' +
                                '<textarea id="sk_remark" name="remark" placeholder="Note" class="form-control" required ></textarea>' +
                            '</div>' +
                        '</div>' +
                    '</form>';
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            btnClass: 'btn-danger',
                            action: function() {
                                let sk_user_code = $('#sk_user_code').val().trim();
                                let remark = $('#sk_remark').val().trim();
                                if (sk_user_code == '') {
                                ...
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village_v2/NcVillageCoController/revertVillage',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village_v2/NcVillageCoController/verifyDraftChitha',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
                ...
                <div class="card-body">
                    ...
                    <div class="border-top border-dark col-lg-12 col-md-12 py-3">
                        <div class="form-group">
                            ...
                            <textarea x-model="co_certification" placeholder="CO Certification" rows="2" class="form-control"></textarea>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="border-top border-dark col-lg-12 col-md-12 py-3" x-show="nc_village.co_verified != 'Y' && nc_village.status != 'H'">
                        ...
                        <button x-on:click="revertVillage" class="btn btn-danger" type="button"><i class="fa fa-backward"></i> Revert Back to LRS</button>
                    </div>
                    ...
                    <div x-show="nc_village.status == 'H'">
                        <h5 class="text-danger text-center">This village has been reverted back to LRS</h5>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            <?php foreach ($maps as $key => $map_single) : ?>
                                <tr>
                                    <td><?= $key + 1 ?></td>
                                    <td>
                                        <a href="<?= base_url() . 'index.php/nc_village_v2/NcVillageCoController/viewUploadedMap?id=' . $map_single->id ?>" class="btn btn-info py-2" style="color: white" target="_blank">
                                            View Map
                                        </a>
                                    </td>
                                </tr>
