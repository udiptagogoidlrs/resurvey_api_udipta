    ...
    function alpineData() {
        return {
            ...
            init() {
                ...
                var self = this;
                this.$watch('selected_team', function() {
                    self.getSurveyors();
                ...
                //     self.vill_townprt_code = '';
                // });
                this.$watch('cir_code_loc', function() {
                    self.getMouzas();
                    self.mouza_pargona_code = '';
                    self.lot_no = '';
                    self.vill_townprt_code = '';
            ...
            saveSurveyorVillages() {
                ...
                self.is_saving = true;
                var vills = {};
                // console.log(self.selected_villages);
                // console.log(self.villages);
                self.selected_villages.forEach(vill_townprt_code => {
                    var obj = {[vill_townprt_code]: 0};
                    obj.vill_townprt_code = vill_townprt_code;
                    obj.apprx_area = 0;
                    self.villages.forEach(element => {
                        if (element.vill_townprt_code == vill_townprt_code) {
                            // obj.apprx_area = element.apprx_area;
                            obj = {[vill_townprt_code]: element.apprx_area};
                        }
                    });
                    // vills.push(obj);
                    vills = {...vills, ...obj};
            ...
            getSurveyors() {
                var self = this;
                $.ajax({
                    url: '<?= base_url(); ?>index.php/SurveyorAssignerController/surveyordetails',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
            ...
            getMouzas() {
                var self = this;
                let distSubCir = self.cir_code_loc;

                [self.dist_code, self.subdiv_code, self.cir_code] = distSubCir.split('_');
                $.ajax({
...
<div class="col-lg-12 col-md-12" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9">ASSIGN VILLAGE TO SURVEYOR</div>
    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="row">
                <div class="col-lg-2">
                    <label for="">Select Team</label>
                    <select x-model="selected_team" id="team" class="form-control form-control-sm">
                ...
                <div class="col-lg-2">
                    ...
                    <select x-model="cir_code_loc" id="circle" class="form-control form-control-sm">
                ...
                <div class="col-lg-2">
                    ...
                    <select x-model="selected_surveyor" id="surveyor" class="form-control form-control-sm">
                        ...
                        <template x-for="(surveyor,index) in surveyors" :key="index">
                            <option x-bind:value="surveyor.username"><span x-text="surveyor.name"></span></option>
                        </template>
                    </select>
                </div>
            </div>
        ...
        <div class="col-lg-12 mt-3">
            <div class="card">
                <div class="card-body">
                                ...
                                <template x-for="(village,index) in villages">
                                    <tr>
                                        <td>
                                            ...
                                            <input x-model="selected_villages" type="checkbox" x-bind:id="village.vill_townprt_code+'select'" x-bind:value="village.vill_townprt_code">
                                        </td>
                                        <td x-text="village.district_name"></td>
                                        <td x-text="village.subdivision"></td>
                                        <td x-text="village.circle"></td>
                                        <td x-text="village.mouza_pargona"></td>
                                        <td x-text="village.lot_name"></td>
                                        <td x-text="village.loc_name"></td>
                                        <td x-text="village.vill_townprt_code"></td>
                                        <td>
                                            <input type="text" x-model="village.apprx_area" x-bind:id="village.vill_townprt_code+'area'" placeholder="Enter Approximate Area">
                                        </td>
                                    </tr>
                                </template>
                    ...
                    <button x-on:click="(selected_villages.length > 0 && selected_surveyor != '' && !is_saving) && saveSurveyorVillages" class="btn btn-success" x-bind:class="(selected_villages.length == 0 || selected_surveyor == '' || is_saving) ? 'disabled' : ''" type="button">SAVE CHANGES</button>
