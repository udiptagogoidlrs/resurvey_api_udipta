<div class="card w-100 card-primary">
    ...
    <div class="card-body hide-for-print">
        <div class="row">
            ...
            <div class="col-md-6">
                <label for="vill_townprt_code">Circle</label>
                <select name="cir_code" id="cir_code" class="form-control">
                    <option value="">---Select Circle---</option>
                    <?php foreach($circles as $circle): ?>
                        <option value="<?php echo $circle->dist_code . '-' . $circle->subdiv_code . '-' . $circle->cir_code; ?>" <?php echo ($circle->dist_code . '-' . $circle->subdiv_code . '-' . $circle->cir_code == $circles[0]->dist_code . '-' . $circles[0]->subdiv_code . '-' . $circles[0]->cir_code) ? 'selected' : ''; ?>><?php echo $circle->loc_name . ' (' . $circle->locname_eng . ')' ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="vill_townprt_code" id="vill_townprt_code" class="form-control">
                    ...
                    <?php foreach($villages as $village): ?>
                        <option value="<?php echo $village->uuid; ?>" <?php echo ($village->uuid == $villages[0]->uuid) ? 'selected' : ''; ?>><?php echo $village->loc_name . ' (' . $village->locname_eng . ')' ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
            ...
            <tbody id="view-section-body">
                <?php foreach($verified_dags as $verified_dag): ?>
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-primary" href="<?php echo base_url('index.php/verification/COController/getSignedPdf?param=' . $verified_dag->param); ?>" target="_blank">View</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
...
<script>
    ...
    $(document).on('change', '#cir_code', (e)=>{
        var baseurl = $('#baseurl').val();
        var cir_code = e.currentTarget.value;

        if(cir_code != '' && cir_code != undefined) {
            $.ajax({
                url: baseurl + 'index.php/verification/ADCController/villages',
                method: 'POST',
                ...
                success: function(resp) {
                    ...
                    if(response.length > 0){
                        text += '<option value"">---Select Village---</option>';
                        response.forEach(element => {
                            text += `<option value="${element.uuid}">${element.loc_name} (${element.locname_eng})</option>`;
                        });
                    }
                    $('#vill_townprt_code').html(text);
                }
            });
        }
    });
    ...
    $(document).on('change', '#vill_townprt_code', (e)=>{
        ...
        if(e.currentTarget.value != '' && e.currentTarget.value != undefined) {
            $.ajax({
                url: baseurl + 'index.php/verification/ADCController/viewSignedPdf',
                method: 'POST',
                    ...
                    if(verified_dags.length > 0) {
                        ...
                        verified_dags.forEach(element => {
                            ...
                            var segment5 = `</div>
                                ...
                                <td>
                                    <a class="btn btn-primary" href="${baseurl}index.php/verification/ADCController/getSignedPdf?param=${element.param}" target="_blank">View</a>
                                </td>
                            </tr>`;
