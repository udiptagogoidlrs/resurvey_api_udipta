...
<script>
	function alpineData() {
		return {
			...
			'villages': [],
			'base_url': "<?= base_url(); ?>",
			'NC_VILLAGE_PROPOSAL_DIR': "<?= NC_VILLAGE_PROPOSAL_DIR . 'dc/'; ?>",
			'is_loading': false,
			...
			init() {
				...
				$.ajax({
					url: '<?= base_url(); ?>index.php/nc_village/NcVillageDcController/getProposalVillages',
					method: "POST",
					async: true,
					dataType: 'json',
					data: data,
			...
			signProposal() {
				...
				$.ajax({
					url: '<?= base_url(); ?>index.php/nc_village/NcVillageDcController/saveProposalPdf',
					method: "POST",
					async: true,
					dataType: 'json',
					data: data,
					success: function(data) {
						...
						if (data != null || data != '') {
							...
							self.loadPreviewForSign('<?= base_url() ?>index.php/nc_village/NcVillageDcController/getProposalBase');
			...
			storeSignedProposal(jsonData, sign_key) {
				...
				$.ajax({
					url: '<?= base_url(); ?>index.php/nc_village/NcVillageDcController/storeSignedProposal',
					method: "POST",
					async: true,
					dataType: 'json',
					data: {
			...
			sendProposalSignByPass() {
				var self = this;

				$('#loader2').removeClass('invisible');

				$.ajax({
					url: '<?= base_url(); ?>index.php/nc_village/NcVillageDcController/sendProposalSignByPass',
					method: "POST",
					async: true,
					dataType: 'json',
					success: function(data) {
...
<div class="col-lg-12 col-md-12" x-data="alpineData()">
	...
	<div class="row justify-content-center">
		<div class="col-lg-12">
			<div class="row">
				<div class="col-lg-3">
					<label for="">Proposal Request Type</label>
			...
			<div class="card">
				<div class="card-body">
							...
							<tbody>
								...
								<template x-for="(village,index) in villages" :key="index">
									<tr x-show="request_type == '' ||
										...
										<td x-text="village.co_verified_at ? (new Date(village.co_verified_at)).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : ''"></td>
										...
										<td>
											<a class="btn btn-sm btn-info text-white no-wrap" x-bind:href=base_url+"index.php/nc_village/NcVillageDcController/showDags?application_no="+village.application_no target=" _blank">View <i class=" fa fa-eye"></i></a>
										</td>
									</tr>
								</template>
					...
					<div class="mt-3" x-show="villages.length > 0">
						<button class="btn  btn-md btn-success" x-bind:class="(selected_cases.length == 0) ? 'disabled' : ''" x-on:click="(selected_cases.length > 0) ? approveProposal : 'javascript:void(0)'">
		...
		<div class="col-lg-12 mt-3">
			<div class="card">
				<div class="card-body">
					<div class="d-flex justify-content-between align-items-center mb-1">
						<div>
							<h5> List of approved proposals</h5>
						</div>
					</div>
					<table class="table table-hover table-sm table-bordered table-stripe">
						<thead class="bg-warning">
						...
						<tbody>
							...
							<tr x-show="approve_proposal.length == 0">
								<td colspan="6" class="text-center">
									<span>No Approved Proposal Found</span>
								</td>
							</tr>
			...
			<div class="modal-dialog modal-xl" style="width: 100% !important;">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" x-on:click="clearSignData" class="btn btn-danger" id="closeBtn" data-dismiss="modal"><i class='fa fa-close'></i> Close
						</button>
