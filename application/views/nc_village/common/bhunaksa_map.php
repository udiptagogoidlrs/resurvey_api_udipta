...
<script>
    ...
    $(document).ready(function() {
        ...
        $.post("<?= base_url()?>index.php/nc_village/NcVillageCommonController/viewBhunaksaMapPost", {
            location: "<?= $location ?>"
        }, function(data) {
            ...
            villVector = new ol.layer.Vector({
                source: new ol.source.Vector({
                    format: format
                }),
                style: function (feature, resolution) {
                    var textValue = feature.get('kide');
                    var intValue = parseInt(textValue);

                    // Highlight dags in the specified range only if highlightDags is true
                    var isHighlighted = false;
                    if (highlightDags && minDag !== null && maxDag !== null && !isNaN(intValue)) {
                        isHighlighted = intValue >= minDag && intValue <= maxDag;
                    }
            ...
            villVector.getSource().addFeatures(features);
            ...
            map.on('pointermove', function(evt) {
                ...
                if (feature) {
                    ...
                    var textValue = feature.get('kide');
                    var intValue = parseInt(textValue);
                    var areaValue = parseFloat(feature.get('Area')).toFixed(4);
