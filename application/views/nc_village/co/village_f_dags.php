    ...
    function alpineData() {
        return {
            ...
            init() {
                ...
                var nc_village = <?= json_encode($nc_village, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) ?>;
                // nc_village = nc_village.replace('\n', '\\n');
                // nc_village = JSON.parse(nc_village);
            ...
            getDags() {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/getDags',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/verifyDag',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            certifyVillage() {
                if (!this.change_village_name) {
                    alert('Please change village name before proceeding');
                    return;
                }
                if (!this.co_note) {
                    alert('Please enter the CO Note');
                    return;
                }
                if (!this.co_certification) {
                    alert('Please enter the CO Certification');
                    return;
                }
                var self = this;
                this.is_loading = true;
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/certifyVillage',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
                                        'application_no': self.application_no,
                                        'co_certification': self.co_certification,
                                        'remark': self.co_note,
                                    ...
                                    success: function(data) {
                                        ...
                                        if (data.submitted == 'Y') {
                                            $.confirm({
                                                title: 'Success',
                                                content: 'The draft Chitha and map of this village have been certified successfully.',
                                                type: 'green',
                                                typeAnimated: true,
                                                buttons: {
                                                    Ok: {
            ...
            revertVillage() {
                ...
                $.confirm({
                    title: 'Confirm',
                    content: 'Please confirm to Revert Back this Village to SK',
                    type: 'orange',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/revertVillage',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            forwardToSkVillage() {
                ...
                $.confirm({
                    title: 'Confirm',
                    content: 'Please confirm to send this Village to SK for Chitha Correction',
                    type: 'orange',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/verifyDraftChitha',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
...
<div class="col-lg-12 col-md-12" x-data="alpineData()">
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-lg-12 mt-3">
            <div class="card">
                <div class="card-body">
                    <form action="<?= base_url() ?>index.php/nc_village/NcVillageCommonController/viewBhunaksaMap" method="post">
                        ...
                        <span>
                            ...
                            <input type="hidden" name="case_type" id="case_type" value="<?= $nc_village->case_type ?>">
                            <?php if ($nc_village->case_type == 'NC_TO_C'): ?>
                        ...
                        <table class="table table-striped table-hover table-sm table-bordered">
                            <thead style="position: sticky;top:0;" class="bg-warning">
                                <tr>
                                    ...
                                    <th>Occupiers</th>
                                    <th>Area(B-K-L)</th>
                                    <th>Verified By SK <br>(Draft Chitha & Map)</th>
                                    <th>Verified By SK</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    ?>
                        <div id="merge_village_data">
                            <?php if ($nc_village->case_type == 'NC_TO_NC') : ?>
                    ...
                    <div class="border-top border-dark col-lg-12 col-md-12 py-3" x-show="nc_village.co_verified != 'Y' && nc_village.status != 'H'">
                        ...
                        <button x-on:click="revertVillage" class="btn btn-danger" type="button"><i class="fa fa-backward"></i> Revert Back to SK</button>
                    </div>
                    <div x-show="nc_village.co_verified == 'Y'">
                        <h5 class="text-success  text-center">The Draft Chitha and Map of this village has already been certified.</h5>
                    </div>
                    <div x-show="nc_village.status == 'H'">
                        <h5 class="text-danger text-center">This village has been reverted back to SK</h5>
                    </div>
                    <div class="border-top border-dark col-lg-12 col-md-12 py-3" x-show="nc_village.co_verified == 'Y' && nc_village.status == 'h'">
                        <div class="form-group">
                            <label class="form-label font-weight-bold">
                                CO Note <span class="text-danger">*</span>
                            </label>
                            <textarea x-model="co_note" placeholder="Enter CO Note..." rows="3" class="form-control border-primary rounded shadow-sm"></textarea>
                        </div>
                        <button x-on:click="forwardToSkVillage" class="btn btn-outline-info btn-md px-4 mt-2" type="button">
                            <i class="fa fa-undo"></i> Forward to SK for Chitha Correction
                        </button>
                    </div>
                </div>
            </div>
                                        ...
                                        <?php else : ?>
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                                <div class="form-group">
                                                    <label for="sel1">New Village Name:</label>
                                                    <?php if ($nc_village->case_type == 'NC_TO_C'): ?>
                                                        <input value="<?php echo htmlspecialchars($merge_village_requests[0]['vill_loc']['village']['loc_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" name="new_vill_name" type="text" class="form-control" id="new_vill_name" required readonly>
                                                    <?php else: ?>
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                                <div class="form-group">
                                                    <label for="sel1">New Village Name (Eng):</label>
                                                    <?php if ($nc_village->case_type == 'NC_TO_C'): ?>
                                                        <input value="<?php echo htmlspecialchars($merge_village_requests[0]['vill_loc']['village']['locname_eng'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" name="new_vill_name_eng" type="text" class="form-control" id="new_vill_name_eng" required readonly>
                                                    <?php else: ?>
                                    ...
                                    <tbody>
                                        <?php foreach ($maps as $key => $map_single) : ?>
                                            <tr>
                                                <td><?= $key + 1 ?></td>
                                                <td>
                                                    <a :href="'<?= base_url() ?>index.php/nc_village/NcVillageCoController/viewUploadedMap?id=' + <?= $map_single->id ?>"
                                                        class="btn btn-outline-info btn-sm" target="_blank">
                                                        <i class="fa fa-eye"></i> View Map
                                                    </a>
                                                </td>
                                                <td>
                                                    <?= isset($map_single->created_at) ? date('d-m-Y h:i A', strtotime($map_single->created_at)) : '' ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                        <?php if (count($maps) == 0) : ?>
                        ...
                        <div class="card border-secondary">
                            ...
                            <div class="card-body p-2">
                                <table class="table table-striped table-hover table-sm table-bordered mb-0">
                                    ...
                                    <tbody>
                                        <?php if (count($old_maps) > 0): ?>
                                            <?php foreach ($old_maps as $key => $old_map): ?>
                                                <tr>
                                                    <td><?= $key + 1 ?></td>
                                                    <td>
                                                        <a :href="'<?= base_url() ?>index.php/nc_village/NcVillageCoController/viewUploadedMap?id=' + <?= $old_map->id ?>"
                                                            class="btn btn-outline-info btn-sm" target="_blank">
                                                            <i class="fa fa-eye"></i> View Map
                                                        </a>
                                                    </td>
                                                    <td><?= date('d-m-Y h:i A', strtotime($old_map->created_at)) ?></td>
                                                    <td><?= $old_map->dc_signed ? 'Yes' : 'No' ?></td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr>
                                                <td colspan="5" class="text-center">No Old Maps Found</td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
