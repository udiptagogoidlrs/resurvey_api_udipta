    ...
    function alpineData() {
        return {
            ...
            getVillages() {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcVillageCoController/getVillagesSmH',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div>
                            <h5 class="mb-1 text-primary font-weight-bold">
                                List of Villages Pending for Chitha Correction (Forwarded by Joint Director of Survey)
                            </h5>
                            <small class="text-dark" x-text="'Total Villages: ' + villages.length"></small>
                            <span class="mx-1">|</span>
                            <small class="text-warning" x-text="getPendingVillages + ' Pending'"></small>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover table-sm table-bordered table-stripe">
                            <thead class="bg-warning">
                                <tr>
                                    ...
                                    <th class="no-wrap">Merge Villages Name</th>
                                    <th class="no-wrap">JDS Forwarded at</th>
                                    <th class="no-wrap">JDS Note</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <template x-for="(village,index) in villages">
                                    <tr>
                                        ...
                                        <td>
                                            <span x-text="village.updated_at ? (new Date(village.updated_at)).toLocaleString() : 'N.A.'"></span>
                                        </td>
                                        <td x-text="village.jds_note"></td>
                                        <td>
                                            <b>
                                                <small class="text-warning" x-show="village.status == 'h'">Pending Correction</small>
                                            </b>
                                        </td>
                                        <td>
                                            <a class="btn btn-sm btn-info text-white no-wrap" x-bind:href=base_url+"index.php/nc_village/NcVillageCoController/showDags?application_no="+village.application_no>Proceed <i class=" fa fa-chevron-right"></i></a>
                                        </td>
                                    </tr>
                                </template>
